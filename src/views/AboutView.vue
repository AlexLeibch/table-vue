<template>
  ...
  <table border="1px" cellpadding="3px" rules="groups rows" align="center">
    <thead>
      <tr>
        <th @click="() => setSortedList('id')" >Место {{setFingerVision('id') }}</th>
        <th @click="() => setSortedLogin('login')">Логин{{setFingerVision('login') }}</th>
        <th @click="() => setSortedList('order')">Подтвержденные заказы {{setFingerVision('order') }}</th>
        <th @click="() => setSortedLogin('status')">Статус {{setFingerVision('status') }}</th>
      </tr>
    </thead>
    <tbody>
...
export default {
  ...
  computed: {
    setSearchLogin() {
      ...
      return [...this.data].filter((info) => info.login.toLowerCase().includes(this.searchQuery.toLowerCase())
      );
    },
    setSearchStatus() {
